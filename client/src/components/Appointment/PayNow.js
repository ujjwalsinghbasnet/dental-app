        ...
        fetchServices.verifyPayment(payload).then((data) => {
          if (data.status === 200) {
            toast({
              ...
              description: `Paid ${data.data.amount / 100} to ${
                data.data.merchant.name
              }`,
              status: "success",
              duration: 3000,
              isClosable: true,
    ...
    >
      ...
      >
        ...
        <Flex
          ...
          w="50%"
        >
          <Button variant="primary-btn" onClick={handlePay} w="50%" mr="1rem">
            Pay Now
          </Button>
          <Button variant="secondary-btn" onClick={handleCancel} w="50%">
            close
          </Button>
        </Flex>
      </Box>
