  ...
  return (
    ...
    >
        ...
        <Box 
            ...
            w='100%' top= '0'
            left={ hoverStatus ? '0' : '100%' } padding='1.5rem'
            bg='transparentPrimary' 
            color='white' 
            backdropFilter={'blur(5px)'}
            transition='linear 0.5s'
        >
            <Heading as='h3' fontSize='1.9rem'>{name}</Heading>
            <Text fontSize={'0.9rem'} fontWeight={500}>{designation}</Text>
            <Text textAlign='justify' mt='1.5rem'>{text}</Text>
        </Box>
    </Box>
  );
}
