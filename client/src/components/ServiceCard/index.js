...
function ServiceCard({ title, image, text }) {
  ...
  return (
    ...
    >
        ...
        >
            <Heading as='h3' fontSize='1.9rem' color='white'>{title}</Heading>
        </Box>

        <Box 
            position='absolute' h='100%' 
            w='100%' top={ hoverStatus ? '0' : '100%' } 
            left='0' padding='1rem'
            bg='transparentPrimary' 
            color='white' 
            backdropFilter={'blur(5px)'}
        ...
        >
            <Heading as='h3' fontSize='1.9rem'>{title}</Heading>
            <Text textAlign='justify' mt='0.5rem'>{text}</Text>
        </Box>
    </Box>
  );
}
