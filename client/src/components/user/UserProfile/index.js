...
import { changeDetails,changePassword, reset } from '../../../features/authSlice'
    ...
    if(new_password !== confirm_password){
      ...
      if(!new_password.match(passwordRegex)){
        toast({
          ...
          description: 'must be 6 digit and should consist of number and symbols',
          status: 'error',
          duration: 3000,
          isClosable: true,
        })
      } else {
        dispatch(changePassword({id: user.id || user._id, passwords })).then((data) => {
          if(data.hasOwnProperty('error')){
            toast({
              title: 'Failed',
              description: data.payload.message,
